					...
					<PanelBody title={ __( 'View Settings' ) } initialOpen={ true }>
						{ views.length === 0 && (
							<Fragment>
								...
								<Button href={ st_views.adminURL + 'edit.php?post_type=wpm-testimonial&page=testimonial-views&action=add' } target="_blank" isDefault>{ __( 'Add New View' ) }</Button>
							</Fragment>
						)}
						...
						{ views.length > 0 && (
							<Fragment>
								...
								/>
								{ id != 0 && (
									<Button target="_blank" href={ st_views.adminURL + 'edit.php?post_type=wpm-testimonial&page=testimonial-views&action=edit&id=' + id } isSecondary>{ __( 'Edit View' ) }</Button>
								) }
							</Fragment>
						)}
					</PanelBody>
