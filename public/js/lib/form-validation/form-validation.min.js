 ...
 */
 ...
 function strongValidation(form) {
     ...
     */
    strongValidation.prototype.disableForm = function (currForm) {
    //apply form wait buffer only for the submited form
    	jQuery(currForm).parent().find( '.strong-form-wait' ).show();
		jQuery(currForm).find( '.wpmtst_submit_testimonial' ).prop( 'disabled', true );
    };

    /**
     * Hide overlay after form submission.
     */
    strongValidation.prototype.enableForm = function (currForm) {
		jQuery(currForm).parent().find( '.strong-form-wait' ).hide();
		jQuery(currForm).find( '.wpmtst_submit_testimonial' ).prop( 'disabled', false );
    };
	   ...
	   */
        jQuery(this.form).validate({
            ...
            if (theThis.settings.ajaxUrl !== '') {
                ...
                success: function(success){ theThis.showResponse(success,form); },
                };
	 ...
	 * @param response
	 */
     strongValidation.prototype.showResponse = function (response,currForm) {
	  window.onbeforeunload = null;
	  this.enableForm(currForm);
	  var obj = JSON.parse(response);
	  if (obj.success) {
		jQuery(currForm).parent().html(obj.message);
		this.scrollOnSuccess();
	  } else {
