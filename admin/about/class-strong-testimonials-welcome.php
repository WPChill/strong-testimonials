...
class Strong_Testimonials_Welcome {
	...
	public function redirect_on_activation( $plugin ) {
		...
		if ( WPMTST_PLUGIN == $plugin ) {
			exit( wp_redirect( admin_url( 'index.php?page=wpmtst-getting-started' ) ) );
		}
	}
			...
			<div class="container">
				...
				<div class="hero features">
					...
					<div class="block">
                        ...
                        <?php $welcome->display_extension( esc_html__( 'Multiple views', 'strong-testimonials' ), esc_html__( 'Take advantage of our three testimonial views - display, slider, and form, and use them to apply custom settings to multiple testimonials.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/multiple-views.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Predesigned templates', 'strong-testimonials' ), esc_html__( 'Choose a built-in template, layout, background, and font color to showcase your testimonials in a visually-appealing way.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/predesigned-templates.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Testimonial Importer', 'strong-testimonials' ), esc_html__( 'Keep all your testimonials in one single place by importing reviews from third-party websites/plugins, such as: Google, Facebook, Yelp, Zomato, Woocommerce.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/testimonial-importer.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Testimonial forms', 'strong-testimonials' ), esc_html__( 'Collect testimonials from customers via different types of forms along with their data - name, email, company name, company website, featured image, star rating.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/testimonial-forms.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Country Selector', 'strong-testimonials' ), esc_html__( 'Easily add a form field to allow your customers to submit testimonials with their country information.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/country-selector.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Spam protection', 'strong-testimonials' ), esc_html__( 'Add spam control to your forms with our Captcha extension.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/spam-protection.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Email integration', 'strong-testimonials' ), esc_html__( 'Unlock more marketing and automation potential by sending users a targeted message or a coupon to thank them for leaving a good review and subscribing to a MailChimp email list', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/email-integration.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'SEO-friendly markup', 'strong-testimonials' ), esc_html__( 'Include rating markup to encourage search engines to display rich snippets – whether or not you use a star rating field on your testimonials.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/seo-friendly-markup.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Pending testimonials', 'strong-testimonials' ), esc_html__( 'Manually verify and approve your incoming testimonials, or make use of the auto-approve feature to save time.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/pending-testimonials.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Testimonial assignment', 'strong-testimonials' ), esc_html__( 'Assign testimonials to certain custom post types.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL) . "/img/features/testimonial-assignment.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Testimonial category filters', 'strong-testimonials' ), esc_html__( 'Increase workflow and user experience by filtering testimonials by category, product/service, or star rating.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/testimonial-category-filters.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'Properties ', 'strong-testimonials' ), esc_html__( 'Change properties of the testimonial post type: labels, permalink structure, admin options and post editor features.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/properties.png", true ); ?>
                        <?php $welcome->display_extension( esc_html__( 'User role management', 'strong-testimonials' ), esc_html__( 'As, an admin, easily decide which user roles are worthy of adding, editing, or removing testimonials.', 'strong-testimonials' ),  esc_attr( WPMTST_ADMIN_URL). "/img/features/user-role-management.png", true ); ?>
						...
						<div class="button-wrap clear">
							...
							<div class="right">
                                <?php $welcome->display_button( esc_html__( 'Upgrade Now', 'strong-testimonials' ), 'https://strongtestimonials.com/pricing/?utm_source=welcome_banner&utm_medium=upgradenow&utm_campaign=welcome_banner', true ); ?>
							</div>
						</div>
					</div>
				</div><!-- hero -->
